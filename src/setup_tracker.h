...
/*
  setup_tracker.h  —  APRS Tracker modul pro tvůj projekt
  ...
  Závislosti (jsou už v hlavním kódu):
    #include <Arduino.h>
    #include <LoRa.h>
  ...
  Externí symboly z hlavního kódu (už je máš):
    extern AsyncWebServer server;
    extern Adafruit_SSD1306 display;
    extern NTPClient timeClient;
    extern WiFiClient client;
    extern String web_username, web_password;
    extern String call;              // iGate/DIGI callsign
    extern String lastStation;       // poslední RX stanice pro OLED spodní řádek
...
// ---------- Konfigurace / výchozí hodnoty ----------
  ...
  #define GPS_RX_PIN_DEFAULT 16
#endif
#ifndef GPS_TX_PIN_DEFAULT
  #define GPS_TX_PIN_DEFAULT 17
#endif
...
static void _encodeCompressed(double lat, double lon, char out8[9]) {
  // podle APRS spec: Y = 90 - lat; X = 180 + lon
  // LAT: floor(380926 * (90 - lat)), LON: floor(190463 * (180 + lon))
...
static void _loadCfg() {
  ...
  // --- line 1 ---
  {
    ...
    t=_nextToken(s); /* legacy interval – ignorujeme */
...
static String _buildPayload_Uncompressed() {
  // !DDMM.mmN{table}DDDMM.mmE{sym} comment
  String payload = "!";
...
static uint32_t _smartInterval(double vKmh) {
  ...
  // lineární interpolace mezi slowRate..fastRate
...
static void _parseRMC(const String& line) {
  // $GPRMC,hhmmss.s,A,llll.ll,a,yyyyy.yy,a,x.x,x.x,ddmmyy,...*CS
  //         1        2 3        4 5         6  7   8   9
...
static void _oledDraw() {
  ...
  // 2.–3. řádek: pozice/rychlost/výška
  ...
  // poslední řádek: RX info (z hlavního kódu)
...
static void _startTracker() {
  ...
  _running = true;
  _cfg.enabled = true;
  _saveCfg();      // uloží i „poslední stav“
  _lastTxMs = 0;
...
static void tracker_setup(AsyncWebServer* srv) {
  _loadCfg();
  // Obnova posledního stavu
  if (_cfg.enabled) _startTracker();

  // Pokud máme server, zaregistruj UI
  if (srv) {
    ...
    srv->on("/tracker", HTTP_POST, [](AsyncWebServerRequest *req){
      ...
      _saveCfg();
      // pokud tracker běží, přenastav UART bez restartu
      if (_running) _applyGpsSerial();
